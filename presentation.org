...
** Bulk properties
Questions for Dr. Widom:
- Currently, we are performing bulk MC with only 108 atoms (54 Cu and 54 Pd atoms), but I am unsure if this would be considered sufficient.
- MCs are being performed to \approx 15,000 (successful) iterations with \approx 35% success rate. What criteria are most commonly accepted for convergence?
- MC code is custom written to perform swaps on only Cu and Pd atoms to improve efficiency. Are there any other standard methods for improving performance?
...
- Is relaxation typically considered for MC simulation?
...
*** Next steps: Semi-Grand Canonical MC simulations
...
-- Other sources seem to imply that chemical potentials from the ground state hull are used at all temperatures?
