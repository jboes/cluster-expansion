...
#jb.write_database('DFT/bulk=fcc/lattice=3.634', db='data36.db')

with connect('networks/db4-bulk/data.db') as mdb:

    db = connect('data.db')

    for d in db.select():
        atoms = db.get_atoms(d.id)
        mdb.write(atoms, key_value_pairs=d.key_value_pairs)

    db = connect('data36.db')
...
db = connect('networks/db4-bulk/data.db')
...
db0 = connect('networks/db4-bulk/train.db')
...
for r, d, f in os.walk('DFT/surf=fcc'):
    if f:
        k = [x for x in r.split('/') 
             if '=' in x
             and 'host' in x
             or 'lattice' in x
             or 'type' in x]
        k[0] = {k[0].split('=')[0]: str(k[0].split('=')[1])}
        k[1] = {k[1].split('=')[0]: float(k[1].split('=')[1])}
        k[2] = {'layers': int(k[2][-1])}
#+END_SRC

#+RESULTS:
: [{'host': 'Pd'}, {'lattice': 3.842}, {'layers': 2}]
...
#+RESULTS:
: {'db3-7': -145.4988296599197, 'db3-6': -145.70377985147238, 'db2-10': -145.46757047772155, 'db2-12': -147.22481869781907, 'db3-8': -145.67327524825865}

* Starting ASAP screening approach for bulk structures
            ...
            with jasp(r) as calc:
                try:
                    ...
                    k = [x for x in r.split('/') 
                         ...
                         or 'lattice' in x
                         or 'type' in x]
                    kv[k[0].split('=')[0]] = str(k[0].split('=')[1])
                    kv[k[1].split('=')[0]] = float(k[1].split('=')[1])
                    kv['layers'] = int(k[2][-1])

                    for i, atoms in enumerate(traj):

                        keys = kv
                        keys.update({'traj': int(n-i-1),
...
#+RESULTS:
: <type 'exceptions.ValueError'>: DFT/surf=fcc/host=Cu/lattice=3.634/type=layers2/cfg=314/xy=1.00
: 
...
mdb = connect('temp/master.db')

hashs = []
for f in os.listdir('temp'):
    db = connect('temp/' + f)
    for d in db.select():
        if d.hash not in hashs:
            hashs += [d.hash]
            data = d.data
            data.update({'hash': d.hash})
            mdb.write(d, data=d.data)
#+END_SRC
...
db = connect('temp/pred-4layer.db')

for d in db.select(['id<=5']):
    data = d.data
    data.update({'hash': d.hash})
    print data
#+END_SRC
...
#+RESULTS:
: {u'NN12': -124.0128352084126, u'NN10': -124.05190576982467, 'hash': u'6de1471905e9fc073ed94d3a442c4ef8'}
: {u'NN12': -125.98583428205852, u'NN10': -126.0117982174777, 'hash': u'15896e94868dc62fe74851ae653bc5d1'}
: {u'NN12': -127.75829229726118, u'NN10': -127.77166324045552, 'hash': u'd83a579814f1be70f08c9d4787b038ff'}
: {u'NN12': -129.41069015743577, u'NN10': -129.40386527602988, 'hash': u'869139452a1217aa3961c39ec0362b95'}
: {u'NN12': -130.89038691106876, u'NN10': -130.87507637543385, 'hash': u'edddc8aa98494a75932bc88baf75a775'}
...
#+caption: Second iteration of NN predicted energy differences for all unique energy configurations shown above which have a unique hash.
...
plt.title('{0} total calculations'.format(len(E)))
plt.tight_layout()
plt.savefig('./images/db1-alayer.png')
#+END_SRC
...
for f in factors:
    for M in ['Cu', 'Pd']:
        ...
        #atoms = fcc111_root(M, 7, size=(1, 1, 1), vacuum=6.0, a=f)
        atoms = fcc111(M, size=(1, 1, 5), vacuum=6.0, a=f)
        n = len(atoms)

        calc = Amp('networks/db3/40-6-6-1/')
        atoms.set_calculator(calc)
        nrg1 = atoms.get_potential_energy()

        calc = Amp('networks/db3/40-7-7-1/')
        atoms.set_calculator(calc)
        nrg2 = atoms.get_potential_energy()
        atoms.set_calculator(None)

        print '{0:1.6f}'.format(abs(nrg2 - nrg1) / n)
#+END_SRC
...
** Predictions with 5 layer 2 \times 2 slab
...
** Predictions with 5 layer root(21) \times root(21) slab
#+BEGIN_SRC python :results silent
from itertools import product
from ase.lattice.surface import fcc111_root
from asap3 import EMT
...
base = fcc111_root('Cu', 21, size=(1, 1, 5), vacuum=6.0, a=3.634)
natoms = [a.index for a in base if a.tag <= 1]

with connect('traj/cfgr21x5-1LE.db') as db:
    nrgs = set()
    for i, c in enumerate(product(['Cu', 'Pd'],
                                  repeat=len(natoms))):
        atoms0 = Atoms.copy(base)
        for j, s in enumerate(natoms):
            atoms0[s].symbol = c[j]
...
images = read('traj/cfgr21x5-1LE.db')

with connect('temp/cfgr21x5-1LE-db3.db') as db:
    data = {}
    for atoms in images:
        for l in [6, 7]:
...
#+caption: db3 NN predicted differences in energy for all root(21) \times root(21) unique energy configurations of a 5 layer fcc(111) slab.
#+label: fig-db3-cfgr21x5-1LE
#+attr_latex: :width 4in :placement [H]
#+attr_org: :width 400
[[./images/cfgr21x5-1LE-db3.png]]

** Predictions with 5 layer 3 \times 3 slab
#+BEGIN_SRC python :results silent
from itertools import product
from ase.lattice.surface import fcc111
from asap3 import EMT
...
base = fcc111('Cu', size=(3, 3, 5), vacuum=6.0, a=3.634)
natoms = [a.index for a in base if a.tag <= 3]

with connect('traj/cfg3x5-3LE.db') as db:
    nrgs = set()
    for i, c in enumerate(product(['Cu', 'Pd'],
                                  repeat=len(natoms))):
        atoms0 = Atoms.copy(base)
...
** Predictions with 6 layer root(7) \times root(7) slab
#+BEGIN_SRC python :results silent
from ase.db import connect
from ase.io import read
from ase.lattice.surface import fcc111_root
...
with connect('traj/cfgr7x6-4LE.db') as db:
    for atoms in images:

        sym = atoms.get_chemical_symbols()

        base = fcc111_root(sym[0], 7,
                           size=(1, 1, 6),
                           vacuum=6.0, a=3.634)
        natoms = [a.index for a in base if a.tag <= 5]
...
images = read('traj/cfgr7x6-4LE.db')

with connect('temp/cfgr7x6-4LE-db3.db') as db:
    data = {}
    for atoms in images:
        for l in [6, 7]:
...
for r, d, f in os.walk('networks/db4/'):
    ...
    if 'trained-parameters.json' in f:
        with open(os.path.join(r, 'trained-parameters.json'), 'r') as fi:
            p = json.load(fi)
        n = p['hiddenlayers']
        print('|{0}|{1}|{2}|{3}|{4}|'.format(n, v[0], v[1], v[2], v[3]))
#+END_SRC

#+RESULTS:
| Hidden layers                    | Iteration | Time                | Cost Function | Energy RMSE |
|----------------------------------+-----------+---------------------+---------------+-------------|
| {u'Cu': [2, 10], u'Pd': [2, 10]} |     21925 | 2016-01-23T22:36:19 |     1.528e-02 |   8.182e-04 |
| {u'Cu': [2, 8], u'Pd': [2, 8]}   |     18925 | 2016-01-22T22:00:40 |     1.166e-02 |   7.148e-04 |
