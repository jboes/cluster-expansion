...
for d in db.select():
    ...
    db.update(d.id, NN10=nrg10, NN12=nrg12, amp_hash=H)
#+END_SRC
...
** Predictions of 3-layer configurations
...
plt.xlabel('Predicted energy difference (eV)')
plt.ylabel('Probability')
plt.xlim(0, nbins(max(E)).max())
plt.title('{0} total calculations'.format(len(E)))
plt.tight_layout()
plt.savefig('./images/db0-3layer.png')
#+END_SRC

#+RESULTS:
: 2879

** Predictions for stress changes
...
images = read('traj/unique-3layer.db')

with connect('traj/unique-3layer.db') as db:
    for atoms in images:
        cell = atoms.get_cell()
        f = 1.0/1.08392955421
