...
db = connect('networks/db1-nrg0/data.db')

cfg = {}
cfg['fcc'], cfg['bcc'] = [], []
for d in db.select():
    if d.bulk == 'fcc':
        cfg['fcc'].append(d.config)
    if d.bulk == 'bcc':
        cfg['bcc'].append(d.config)
...
for key, value in cfg.iteritems():
    for v in value:
        ...
        data = db.select(['bulk={0}'.format(key),
                          'config={0}'.format(v)])
        ...
        '''
        plt.savefig('images/3D-EOS-{0}-cfg{1}.png'.format(key, v))
#+END_SRC
