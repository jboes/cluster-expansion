...
full_traj = connect('traj/full.db')
uniq_traj = connect('traj/uniq.db')
...
for cfg in perms:
    ...
    nrgs.append(round(atoms.get_potential_energy(), 3))
...
base = fcc111_root('Cu', 3, size=(1, 1, 5), vacuum=6.0, a=3.787)
