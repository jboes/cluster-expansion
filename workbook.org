...
#save = connect('traj/unique-1layer.db')
#calc = BPNeural(json='networks/db2-surf/l3n30i0/parameters-checkpoint.json')

base = fcc111_root('Cu', 7, size=(1, 1, 5), vacuum=6.0, a=3.634)
natoms = [a.index for a in base if a.tag <= 1]  + [25]
metals = ['Cu', 'Pd']
...
for i, c in enumerate(product(metals, repeat=len(natoms))):
    ...
    if nrg not in nrgs:
        nrgs.add(nrg)
        write('./images/cfg{0}.png'.format(i), atoms0*(3, 3, 1))
print len(nrgs)
'''
...
Number of unique permutations predicted for 3 layers of root 7 x root 7 slab.
...
basis = [['Cu', 3.634, np.linspace(1.00, 1.08392955421, 4)],
         ['Pd', 3.939, np.linspace(1.00, 1.0/1.08392955421, 4)]]
...
for M, a, factors in basis:
    ...
    for cfg in range(343): # The number of unique 2-layer configurations
        ...
        for f in factors:
            ...
            atoms0.set_cell(np.dot(cell0, delta), scale_atoms=True)
            lat = atoms0.get_distance(2, 10)

            for xy in [0.98, 1.00, 1.02]:
                ...
                d2 = np.array([[xy, 0, 0],
                               [0, xy, 0],
                               [0, 0, 0]])
                atoms.set_cell(np.dot(cell0, d2), scale_atoms=True)

                wd = './DFT/surf=fcc/host={0}/lattice={1:1.3f}/type=layers2/cfg={2}/xy={3:1.2f}'.format(M, lat, cfg, xy)
...
wd = 'DFT/surf=fcc/host=Cu/lattice=3.634/type=layers2/cfg=0/xy=1.00'
