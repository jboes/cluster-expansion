...
#print np.array(list(set(product([0, 1, 2], repeat=3))))

n = []
for i, l in enumerate(product(['Cu', 'Pd'], repeat=7)):
    n[i] = []
...
for r, d, f in os.walk('./DFT/surf=fcc/'):
    if f:
        try:
            ...
            UF += [r]
            jb.reset(r)
print '{0} calculations complete'.format(len(nrg))
