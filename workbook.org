...
for d in db.select(['energy<-50.0']):
    print d.key_value_pairs

#+END_SRC

#+RESULTS:
: {u'kpt1': 10, u'bulk': u'bcc', u'kpt3': 10, u'kpt2': 10, u'train_set': True, u'ediff': 1e-08, u'xc': u'PBE', u'calc_time': 10051.138, u'encut': 400.0, u'traj': 0, u'volume': 262.00844435309403, u'strain': u'xyz', u'gga': u'PS', u'total_energy': -233.84348643, u'factor': 1.85, u'fermi': -2.8068, u'config': 7, u'kppra': 1000}
: {u'kpt1': 13, u'bulk': u'bcc', u'kpt3': 3, u'kpt2': 13, u'train_set': True, u'ediff': 1e-08, u'xc': u'PBE', u'calc_time': 32668.14, u'encut': 400.0, u'traj': 0, u'volume': 587.6594108821723, u'strain': u'xyz', u'gga': u'PS', u'total_energy': -331.72696322, u'factor': 2.0, u'fermi': 3.0272, u'config': 105, u'kppra': 507}
: {u'kpt1': 13, u'bulk': u'bcc', u'kpt3': 3, u'kpt2': 13, u'train_set': True, u'ediff': 1e-08, u'xc': u'PBE', u'calc_time': 33269.739, u'encut': 400.0, u'traj': 0, u'volume': 591.9836678257059, u'strain': u'xyz', u'gga': u'PS', u'total_energy': -1123.20902124, u'factor': 1.9, u'fermi': -11.0714, u'config': 112, u'kppra': 507}
: {u'kpt1': 13, u'bulk': u'bcc', u'kpt3': 3, u'kpt2': 13, u'train_set': True, u'ediff': 1e-08, u'xc': u'PBE', u'calc_time': 37317.586, u'encut': 400.0, u'traj': 0, u'volume': 639.9598923540368, u'strain': u'xyz', u'gga': u'PS', u'total_energy': -14475.97504878, u'factor': 1.95, u'fermi': -5.0675, u'config': 112, u'kppra': 507}
: {u'kpt1': 8, u'bulk': u'bcc', u'kpt3': 5, u'kpt2': 8, u'train_set': True, u'ediff': 1e-08, u'xc': u'PBE', u'calc_time': 16549.096, u'encut': 400.0, u'traj': 0, u'volume': 754.6258379257708, u'strain': u'xyz', u'gga': u'PS', u'total_energy': -2011.00383938, u'factor': 1.95, u'fermi': -1.3224, u'config': 618, u'kppra': 320}
...
db0 = connect('networks/db0-PS/data.db')
data = db0.select(['gga=PS', 'ediff!=1e-9'])

wd = 'networks/db0-nbct/'
...
os.chdir('networks/db0-bct')

for L in [4]:
    for N in [30, 40, 50]:
        layers = (L, N)
        label = 'l{0}n{1}'.format(layers[0], layers[1])

        wd = 'train-{0}.py'.format(label)
...
#+RESULTS:
: 1248938.gilgamesh.cheme.cmu.edu
: 1248939.gilgamesh.cheme.cmu.edu
: 1248940.gilgamesh.cheme.cmu.edu
...
reset = []
for n in glob('*train-log*'):
    name = n.split('/')[-1].split('-')[0]
    with open(n) as f:
        lines = f.readlines()
    try:
        itr = lines[-1].split()[0]
        nrg = float(lines[-1].split()[3])
        if int(itr) > 100:
            nrg0 = float(lines[-100].split()[3])
            delta = nrg - nrg0
            if delta > -1e-6:
                reset.append(name)
            else:
                print('{0}-{1:5s} {2:1.5f}  {3:1.6f}'.format(name, itr, nrg, delta))
        else:
            print('{0}-{1:5s} {2:1.4f}'.format(name, itr, nrg))
    except(IndexError):
        print 'iteration:', int(lines[-1].split()[0])
    except(ValueError):
        if  'completed successfully' in lines[-1]:
            print '{0}-{1:5s} converged'.format(name, lines[-5].split()[0])
        elif 'Try it again' in lines[-1]:
            reset.append(name)
        elif 'Saving checkpoint data' in lines[-1]:
            print name, float(lines[-2].split()[3])

for label in reset:
    files = glob('{0}-*'.format(label))
    for f in files:
        os.unlink(f)
...
plt.figure()
plt.plot(ca, nrg, 'ko-', lw=4)

for name in glob('*train-log*'):
    n = name.split('-')[0]
    calc = BPNeural(load='{0}'.format(n))
